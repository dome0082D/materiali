    ...
    <>
      <nav className="bg-white border-b border-rose-100 sticky top-0 z-[5000] shadow-sm flex justify-between items-center h-16 md:h-20 px-4 md:px-8 transition-colors">
        ...
        <div className="flex items-center gap-2 md:gap-4">
          ...
          {!user && (
            <Link href="/login" className="hidden lg:block text-stone-600 font-bold uppercase text-xs md:text-sm tracking-widest hover:text-rose-500 transition-colors px-2">
              Accedi
            </Link>
          )}

          <Link href="/add" className="hidden lg:block bg-gradient-to-r from-rose-500 to-orange-400 text-white px-4 py-2 rounded-xl font-bold uppercase text-xs md:text-sm tracking-widest hover:shadow-lg hover:scale-105 transition-all shadow-md">
            ➕ Vendi
          </Link>
          ...
          <div className="relative">
            <button 
              ...
              className="relative p-2 text-xl md:text-2xl text-stone-500 hover:bg-rose-50 hover:text-rose-500 rounded-full transition-all"
            >
              🔔
              {notifications > 0 && (
            ...
            {isNotifOpen && (
              <div className="absolute right-0 mt-2 w-72 bg-white border border-stone-200 rounded-2xl shadow-xl p-4 z-[6000]">
                <div className="flex justify-between items-center border-b border-stone-100 pb-2 mb-3">
                  <h4 className="text-xs font-bold uppercase tracking-widest text-stone-400">Notifiche</h4>
                  <button onClick={() => setIsNotifOpen(false)} className="text-stone-400 hover:text-stone-800 text-xs font-bold w-6 h-6 flex items-center justify-center rounded-full hover:bg-stone-50">✕</button>
                </div>
                
                {notifList.length === 0 ? (
                  <div className="py-6 text-center">
                    <span className="text-4xl block mb-3">📭</span>
                    <p className="text-xs text-stone-500 font-bold uppercase tracking-widest">Tutto tace</p>
                    <p className="text-[10px] text-stone-400 font-medium mt-1">Nessuna nuova notifica.</p>
                  </div>
                ) : (
          ...
          <div className="relative">
            ...
            }} className="p-2 text-xl md:text-2xl text-stone-500 hover:bg-rose-50 hover:text-rose-500 rounded-full transition-all">
              ⋮
            </button>
            {isQuickMenuOpen && (
              <div className="absolute right-0 mt-2 w-48 bg-white border border-stone-100 shadow-xl rounded-xl p-2 z-[6000]">
                <Link href="/profile" className="block p-3 text-sm font-medium text-stone-700 hover:bg-rose-50 hover:text-rose-600 rounded-lg transition-all">⚙️ Impostazioni</Link>
                {user && (
                  <Link href="/dashboard/analitiche" className="block p-3 text-sm font-bold text-stone-700 hover:bg-rose-50 hover:text-rose-600 rounded-lg transition-all">📈 Seller Hub</Link>
                )}
                <Link href="/supporto" className="block p-3 text-sm font-medium text-stone-700 hover:bg-rose-50 hover:text-rose-600 rounded-lg transition-all">❓ Aiuto</Link>
                {user && (
                  <>
                    <button onClick={handleLogout} className="w-full text-left p-3 text-sm font-medium text-stone-700 hover:bg-stone-50 rounded-lg transition-all">Esci</button>
                    <div className="border-t border-stone-100 my-1"></div>
                    <button onClick={handleDeleteAccount} className="w-full text-left p-3 text-sm font-bold text-red-500 hover:bg-red-50 rounded-lg transition-all">🗑️ Elimina Profilo</button>
                  </>
                )}
              </div>
            )}
          ...
          <button onClick={openCart} className="relative text-2xl md:text-3xl p-2 hover:bg-rose-50 hover:text-rose-500 text-stone-500 rounded-full transition-all focus:outline-none">
            🛒 
            {items.length > 0 && (
              <span className="absolute top-0 right-0 bg-rose-500 text-white text-[9px] w-5 h-5 flex items-center justify-center rounded-full font-bold border-2 border-white shadow-md">
                {items.length}
              </span>
            )}
          </button>
      ...
      {/* -------------------- SIDEBAR (MENU ☰) -------------------- */}
      <div className={`fixed top-0 left-0 h-full w-[85%] max-w-[340px] bg-white z-[9999] shadow-2xl transition-transform duration-300 ease-in-out transform ${isSidebarOpen ? 'translate-x-0' : '-translate-x-full'}`}>
        <div className="flex flex-col h-full">
          <div className="p-6 md:p-8 bg-gradient-to-br from-rose-500 to-orange-500 text-white relative">
            <button onClick={() => setIsSidebarOpen(false)} className="absolute top-4 right-4 text-white/70 hover:text-white text-2xl transition-colors">✕</button>
            ...
            <p className="text-3xl mb-0.5 text-white" style={{ fontFamily: "'Brush Script MT', 'Lucida Handwriting', cursive" }}>Re-love</p>
          ...
          <div className="flex-1 overflow-y-auto p-5 md:p-6 space-y-6">
            <section>
              <h3 className="text-[10px] font-bold uppercase text-stone-400 mb-3 tracking-[0.2em] border-b pb-2 border-stone-100">Area Riservata</h3>
              <div className="grid gap-2">
                <Link href="/add" onClick={() => setIsSidebarOpen(false)} className="block w-full bg-gradient-to-r from-rose-500 to-orange-400 text-white text-center py-3 rounded-xl font-bold uppercase text-[10px] tracking-widest shadow-sm lg:hidden mb-2">➕ Vendi o Regala</Link>
                {user ? (
                  <>
                    ...
                    <button onClick={handleLogout} className="w-full text-left p-3 text-xs font-bold text-red-500 hover:bg-red-50 rounded-xl mt-3 uppercase tracking-widest transition-all">← Esci</button>
                  </>
                ) : (
            ...
            <section className="lg:hidden">
              <h3 className="text-[10px] font-bold uppercase text-stone-400 mb-3 tracking-[0.2em] border-b pb-2 border-stone-100">Strumenti Re-love</h3>
              <div className="grid grid-cols-2 gap-3">
                <button onClick={() => { setDarkMode(!darkMode); setIsSidebarOpen(false); }} className="p-3 text-xs font-bold text-stone-600 bg-stone-50 rounded-xl border border-stone-100 flex flex-col items-center gap-1.5 shadow-sm hover:bg-rose-50 transition-colors">
                  <span className="text-xl">{darkMode ? '☀️' : '🌙'}</span> {darkMode ? 'Chiaro' : 'Scuro'}
                </button>
                <button onClick={() => { setShowSecurityModal(true); setIsSidebarOpen(false); }} className="p-3 text-xs font-bold text-stone-600 bg-stone-50 rounded-xl border border-stone-100 flex flex-col items-center gap-1.5 shadow-sm hover:bg-rose-50 transition-colors">
                  <span className="text-xl">🛡️</span> Scudo
                </button>
                <button onClick={() => { setShowAiModal(true); setIsSidebarOpen(false); }} className="p-3 text-xs font-bold text-stone-600 bg-stone-50 rounded-xl border border-stone-100 flex flex-col items-center gap-1.5 shadow-sm hover:bg-rose-50 transition-colors">
                  <span className="text-xl">🤖</span> Valuta
                </button>
                <button onClick={handleRadar} className="p-3 text-xs font-bold text-stone-600 bg-stone-50 rounded-xl border border-stone-100 flex flex-col items-center gap-1.5 shadow-sm hover:bg-rose-50 transition-colors">
                  <span className="text-xl">📡</span> Radar
                </button>
              </div>
            </section>

            <section>
              <h3 className="text-[10px] font-bold uppercase text-stone-400 mb-3 tracking-[0.2em] border-b pb-2 border-stone-100">Categorie</h3>
              <div className="grid gap-1">
                {categories.map((cat) => (
                  <Link key={cat.id} href={`/?cat=${cat.slug}`} onClick={() => setIsSidebarOpen(false)} className="p-2.5 md:p-3 text-sm font-medium text-stone-600 hover:text-orange-500 hover:bg-orange-50 rounded-xl transition-all capitalize">
                    {cat.name}
                  </Link>
                ))}
              </div>
      ...
      {/* -------------------- CARRELLO (IMMAGINI PIÙ GRANDI) -------------------- */}
      <div className={`fixed top-0 right-0 h-full w-[90%] max-w-[400px] bg-white z-[9999] shadow-2xl transition-transform duration-300 ease-in-out transform ${isCartOpen ? 'translate-x-0' : 'translate-x-full'} flex flex-col`}>
        <div className="p-5 md:p-6 flex justify-between items-center border-b border-stone-100 bg-stone-50">
          ...
          {items.length === 0 ? (
            ...
            items.map((item) => (
              <div key={item.id} className="flex gap-4 p-4 bg-white rounded-2xl border border-stone-200 group relative transition-all shadow-sm hover:shadow-md">
                ...
                <img src={(item as any).imageUrl || '/usato.png'} alt={item.title} className="w-20 h-20 md:w-24 md:h-24 object-cover rounded-xl border border-stone-200" />
                <div className="flex-1 flex flex-col justify-between">
                  <h3 className="font-bold text-sm text-stone-800 line-clamp-2">{item.title}</h3>
                  <div className="flex justify-between items-center mt-2">
                    <p className="font-black text-rose-500 text-sm md:text-base">€ {(Number(item.price)).toFixed(2)}</p>
                    <div className="flex items-center gap-3 bg-stone-50 rounded-lg p-1.5 border border-stone-100">
        ...
        {items.length > 0 && (
          <div className="p-5 md:p-6 bg-white border-t border-stone-100">
            <div className="flex justify-between items-center mb-5">
              <span className="text-xs font-bold uppercase tracking-widest text-stone-400">Totale</span>
              <span className="text-2xl font-black text-rose-600">€ {total.toFixed(2)}</span>
            </div>
            <button onClick={handleCheckout} disabled={loading} className="w-full bg-gradient-to-r from-rose-500 to-orange-400 text-white py-4 rounded-xl font-black uppercase tracking-widest text-xs shadow-md hover:scale-[1.02] transition-all disabled:opacity-50">
      ...
      {showSecurityModal && (
        <div className="fixed inset-0 z-[10000] flex items-center justify-center p-4">
          <div className="bg-white rounded-[2rem] shadow-2xl p-8 max-w-md w-full relative">
            <button onClick={() => setShowSecurityModal(false)} className="absolute top-4 right-5 text-stone-400 hover:text-stone-800 text-2xl font-bold">✕</button>
            ...
            <div className="space-y-4 text-sm font-medium text-stone-600">
              <p className="flex items-start gap-2"><span className="text-lg">🔒</span> <b>Pagamenti Sicuri:</b> Usiamo Stripe. I fondi sono congelati finché non ricevi il pacco.</p>
              <p className="flex items-start gap-2"><span className="text-lg">🤝</span> <b>Baratto Diretto:</b> Per barattare, usa la chat integrata.</p>
              <p className="flex items-start gap-2"><span className="text-lg">🚚</span> <b>Tracciamento:</b> Tutti gli acquisti "Nuovo" e "Usato" sono tracciati.</p>
            </div>
            <button onClick={() => setShowSecurityModal(false)} className="w-full mt-8 bg-blue-600 text-white py-3.5 rounded-xl font-black uppercase tracking-widest text-xs hover:bg-blue-700 transition-all">Ho Capito</button>
          </div>
        </div>
      )}
      ...
      {showAiModal && (
        <div className="fixed inset-0 z-[10000] flex items-center justify-center p-4">
          <div className="bg-white rounded-[2rem] shadow-2xl p-8 max-w-md w-full relative">
            <button onClick={() => {setShowAiModal(false); setAiResult(null); setAiItemName('');}} className="absolute top-4 right-5 text-stone-400 hover:text-stone-800 text-2xl font-bold">✕</button>
            <div className="text-center mb-6">
            ...
            <input 
              ...
              className="w-full p-4 bg-stone-50 border border-stone-200 rounded-xl mb-4 font-bold text-sm outline-none focus:border-purple-400"
            />
            {aiResult && (
            ...
            <button onClick={handleAiValuation} disabled={loading || !aiItemName} className="w-full bg-purple-600 text-white py-3.5 rounded-xl font-black uppercase tracking-widest text-xs hover:bg-purple-700 transition-all disabled:opacity-50">
      ...
      {showMapModal && (
        <div className="fixed inset-0 z-[15000] bg-white flex flex-col animate-in slide-in-from-bottom duration-500">
          <div className="p-4 border-b border-stone-100 flex justify-between items-center bg-white shadow-sm z-10">
            ...
            <button onClick={() => setShowMapModal(false)} className="bg-stone-900 text-white px-5 py-2.5 rounded-xl font-black uppercase text-[10px] md:text-xs tracking-widest shadow-md">
              Chiudi X
            </button>
          </div>
