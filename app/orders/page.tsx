  ...
  return (
    <div className="min-h-screen bg-stone-50 p-6 md:p-10 pb-32">
      <div className="max-w-4xl mx-auto">
        <h1 className="text-4xl font-black uppercase italic text-stone-900 mb-10">Hub Transazioni</h1>

        {/* SEZIONE PROPOSTE MIGLIORATA */}
        {offersList.length > 0 && (
          <div className="mb-14 space-y-4">
            <h2 className="text-xs font-black uppercase text-stone-400 tracking-[0.2em] mb-4 flex items-center gap-2">
            ...
            {offersList.map(offer => {
              ...
              return (
                <div key={offer.id} className={`p-6 rounded-[2rem] border shadow-sm transition-all flex flex-col md:flex-row justify-between items-start md:items-center gap-6 
                  ...
                  <div className="flex items-center gap-5">
                    ...
                    <div>
                      ...
                      <p className="text-xl font-black text-stone-900 mt-1">€ {offer.offer_price} <span className="text-[10px] text-stone-400 font-bold line-through ml-2">(era €{ann.price})</span></p>
                    </div>
                  </div>
                  ...
                  <div className="w-full md:w-auto">
                    {!isBuyer && offer.status === 'In attesa' ? (
                      <div className="flex gap-3">
                        <button onClick={() => handleOfferAction(offer.id, 'Accettata')} className="flex-1 md:flex-none bg-emerald-500 text-white px-6 py-3 rounded-xl text-[10px] font-black uppercase hover:bg-emerald-600 transition-all shadow-md">✅ Accetta</button>
                        <button onClick={() => handleOfferAction(offer.id, 'Rifiutata')} className="flex-1 md:flex-none border-2 border-rose-200 text-rose-500 bg-white px-6 py-3 rounded-xl text-[10px] font-black uppercase hover:bg-rose-50 transition-all">❌ Rifiuta</button>
                      </div>
                    ) : (
        ...
        <div className="space-y-8">
          {orders.length === 0 ? (
            <div className="text-center py-20 bg-white rounded-[3rem] border border-stone-200 shadow-sm">
              ...
              <p className="text-sm font-medium text-stone-500 mb-8 max-w-sm mx-auto">Non hai ancora completato acquisti o vendite. Il prossimo affare ti sta aspettando là fuori.</p>
              ...
              return (
                <div key={order.id} className="bg-white p-8 rounded-[2.5rem] border border-stone-200 shadow-sm transition-all hover:shadow-lg">
                  <div className="flex justify-between items-start mb-6">
                    <div>
                      <span className={`text-[9px] font-black uppercase px-3 py-1 rounded-lg text-white ${ann.condition === 'Baratto' ? 'bg-blue-500' : 'bg-stone-900'}`}>
                        {ann.condition}
                      </span>
                      <h3 className="text-xl font-black uppercase italic text-stone-900 mt-3">{ann.title}</h3>
                      <p className="text-xs font-bold text-stone-500 uppercase mt-2">
                        Ruolo: <span className="text-stone-900">{isBuyer ? 'Acquirente' : 'Venditore'}</span> | Stato: <span className="text-rose-500">{order.status}</span>
                      </p>
                    </div>
                  ...
                  {(order.status === 'Spedito' || order.status === 'Ricevuto' || order.status === 'Concluso') && isBuyer && order.tracking_number && (
                    <div className="mt-6 p-5 bg-emerald-50 border border-emerald-100 rounded-2xl">
                      ...
                      <div className="grid grid-cols-2 gap-5">
                        <div>
                          <p className="text-[9px] font-black text-stone-500 uppercase">Corriere</p>
                          <p className="text-sm font-bold text-stone-900 mt-1">{order.courier_name || 'In elaborazione...'}</p>
                        </div>
                        <div>
                          <p className="text-[9px] font-black text-stone-500 uppercase">Codice Tracking</p>
                          <p className="text-sm font-bold text-stone-900 mt-1">{order.tracking_number || 'Richiedi in chat'}</p>
                        </div>
                        <div className="col-span-2 pt-4 border-t border-emerald-100/50">
                          <p className="text-[9px] font-black text-stone-500 uppercase">Identificativo Pacco Re-love</p>
                          <p className="text-xs font-mono font-bold text-emerald-800 mt-1">{order.package_id_code}</p>
                        </div>
                      </div>
                      <a 
                  ...
                  {(ann.condition === 'Nuovo' || ann.condition === 'Usato') && isBuyer && (order.status === 'Pagato' || order.status === 'Spedito') && (
                    <div className="mt-8 flex gap-4 pt-6 border-t border-stone-100">
                      <button onClick={() => handleAction(order.id, 'confirm_receipt', role)} className="flex-1 bg-stone-900 text-white p-4 rounded-xl text-[10px] font-black uppercase tracking-widest hover:bg-emerald-500 transition-colors shadow-md">
                        📦 Pacco Ricevuto
                      </button>
                      <button onClick={() => handleAction(order.id, 'request_refund', role)} className="flex-1 border-2 border-stone-200 text-stone-600 bg-white p-4 rounded-xl text-[10px] font-black uppercase tracking-widest hover:border-rose-500 hover:text-rose-500 transition-colors">
                        ❌ Richiedi Rimborso
                      </button>
                    </div>
                  )}
                  ...
                  {ann.condition === 'Baratto' && order.status === 'Pagato' && (
                    <div className="mt-8 pt-6 border-t border-stone-100">
                      {(isBuyer && order.barter_confirmed_buyer) || (!isBuyer && order.barter_confirmed_seller) ? (
                        <p className="text-[10px] font-black uppercase text-orange-500 text-center tracking-widest bg-orange-50 p-5 rounded-xl border border-orange-200">
                          Hai confermato. In attesa dell'altro utente...
                        </p>
                      ) : (
                        <button onClick={() => handleAction(order.id, 'confirm_barter', role)} className="w-full bg-blue-500 text-white p-5 rounded-2xl text-[11px] font-black uppercase tracking-widest hover:bg-blue-600 transition-colors shadow-lg">
                  ...
                  {isBuyer && (order.status === 'Ricevuto' || order.status === 'Concluso') && (
                    <div className="mt-6 pt-6 border-t border-stone-100">
                       <button 
                         ...
                         className="w-full bg-gradient-to-r from-yellow-400 to-amber-500 text-stone-900 p-5 rounded-2xl text-[11px] font-black uppercase tracking-widest hover:scale-[1.02] transition-transform shadow-md"
                       >
                  ...
                  <div className="mt-6 text-center">
                    <Link href={`/chat/${isBuyer ? order.seller_id : order.buyer_id}?ann=${ann.id}`} className="text-[11px] font-black uppercase text-stone-400 hover:text-stone-900 tracking-widest underline decoration-stone-300 underline-offset-4">
                      Apri Chat Annuncio
                    </Link>
                  </div>
                </div>
      ...
      {showReviewModal && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-stone-900/90 backdrop-blur-md">
          <div className="bg-white max-w-md w-full rounded-[3rem] p-10 shadow-2xl border-t-8 border-yellow-400 animate-in zoom-in duration-200">
            <h2 className="text-3xl font-black uppercase italic text-stone-900 mb-2">Com'è andata?</h2>
            <p className="text-sm font-medium text-stone-500 mb-8 italic">La tua opinione aiuta la community a crescere sicura.</p>
            <div className="flex justify-center gap-3 mb-8">
              {[1, 2, 3, 4, 5].map(star => (
                <button key={star} onClick={() => setRating(star)} className={`text-5xl transition-transform hover:scale-110 ${rating >= star ? 'text-yellow-400 drop-shadow-md' : 'text-stone-200'}`}>★</button>
              ))}
            </div>
            <textarea 
              className="w-full p-5 bg-stone-50 border border-stone-200 rounded-2xl text-sm font-medium outline-none focus:border-yellow-400 focus:bg-white transition-all mb-8 min-h-[120px] text-stone-800"
              placeholder="Scrivi un commento..."
              value={comment}
              onChange={(e) => setComment(e.target.value)}
            />
            <div className="space-y-4">
              <button onClick={submitReview} disabled={submittingReview} className="w-full bg-stone-900 text-white py-5 rounded-2xl font-black uppercase text-xs tracking-widest shadow-xl hover:bg-emerald-500 transition-all disabled:opacity-50">
                {submittingReview ? 'Pubblicazione...' : 'Pubblica Recensione'}
              </button>
              <button onClick={() => setShowReviewModal(false)} className="w-full text-stone-400 py-3 rounded-2xl font-black uppercase text-[10px] tracking-widest hover:text-stone-900 bg-stone-50 hover:bg-stone-100 transition-all">
                Annulla e chiudi
              </button>
            </div>
          </div>
